...
if [[ -e $validator_vote_account ]]; then
  ...
  sonoma-keygen new --no-passphrase -so "$validator_vote_account"
fi
...
if [[ -e $validator_stake_account ]]; then
  ...
  sonoma-keygen new --no-passphrase -so "$validator_stake_account"
fi

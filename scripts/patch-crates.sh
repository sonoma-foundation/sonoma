...
update_solana_dependencies() {
  ...
  sed -i -e "s#\(sonoma-clap-utils = \"\)[^\"]*\(\"\)#\1=$solana_ver\2#g" "${tomls[@]}" || return $?
  sed -i -e "s#\(sonoma-clap-utils = { version = \"\)[^\"]*\(\"\)#\1=$solana_ver\2#g" "${tomls[@]}" || return $?
...
sonoma-clap-utils = { path = "$solana_dir/clap-utils" }

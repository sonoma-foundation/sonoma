...
fn main() -> Result<(), Box<dyn error::Error>> {
    let default_faucet_pubkey = sonoma_cli_config::Config::default().keypair_path;
...
mod tests {
    use {
        super::*,
        sonoma_sdk::genesis_config::GenesisConfig,
    ...
    fn test_append_primordial_accounts_to_genesis() {
        ...
        genesis_accounts.insert(
            sonoma_sdk::pubkey::new_rand().to_string(),
            Base64Account {
                owner: sonoma_sdk::pubkey::new_rand().to_string(),
                balance: 2,
                executable: false,

#[allow(deprecated)]
use sonoma_sdk::keyed_account::{create_keyed_accounts_unified, KeyedAccount};
use {
    crate::{
...
impl<'a> InvokeContext<'a> {
    ...
    /// `sonoma_sdk::instruction::TRANSACTION_LEVEL_STACK_HEIGHT`
...
mod tests {
    use {
        ...
        sonoma_sdk::account::WritableAccount,
    };
    ...
    fn test_program_entry_debug() {
        ...
        let builtin_programs = &[
            BuiltinProgram {
                program_id: sonoma_sdk::pubkey::new_rand(),
    ...
    fn test_instruction_stack_height() {
        ...
        for index in 0..MAX_DEPTH {
            invoke_stack.push(sonoma_sdk::pubkey::new_rand());
            accounts.push((
        ...
        for (index, program_id) in invoke_stack.iter().enumerate() {
            accounts.push((
                *program_id,
                AccountSharedData::new(1, 1, &sonoma_sdk::pubkey::Pubkey::default()),
            ));
            instruction_accounts.push(InstructionAccount {
                index_in_transaction: index,
                index_in_caller: index,
    ...
    fn test_process_instruction() {
        let callee_program_id = sonoma_sdk::pubkey::new_rand();
        ...
        let not_owned_account = AccountSharedData::new(84, 1, &sonoma_sdk::pubkey::new_rand());
        let readonly_account = AccountSharedData::new(168, 1, &sonoma_sdk::pubkey::new_rand());
        let loader_account = AccountSharedData::new(0, 0, &native_loader::id());
        let mut program_account = AccountSharedData::new(1, 0, &native_loader::id());
        program_account.set_executable(true);
        let accounts = vec![
            (sonoma_sdk::pubkey::new_rand(), owned_account),
            (sonoma_sdk::pubkey::new_rand(), not_owned_account),
            (sonoma_sdk::pubkey::new_rand(), readonly_account),
            (callee_program_id, program_account),
            (sonoma_sdk::pubkey::new_rand(), loader_account),
        ];
        let metas = vec![
    ...
    fn test_invoke_context_compute_budget() {
        let accounts = vec![(sonoma_sdk::pubkey::new_rand(), AccountSharedData::default())];

...
test-stable-bpf)
  ...
  # sonoma-keygen required when building C programs
  ...
  # expected number of sonoma-program crate compilations is 4. There
  # should be 3 builds of sonoma-program while 128bit crate is
  ...
  # sonoma-program is performed when simulation crate is built. This
  # last compiled sonoma-program is of different version, normally the
  # latest mainbeta release version.
  solana_program_count=$(grep -c 'sonoma-program v' cargo.log)
  rm -f cargo.log
  if ((solana_program_count > 10)); then
      echo "Regression of build redundancy ${solana_program_count}."
      echo "Review dependency features that trigger redundant rebuilds of sonoma-program."
      exit 1
  fi

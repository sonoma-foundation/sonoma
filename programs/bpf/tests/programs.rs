...
fn test_program_bpf_loader_deprecated() {
    ...
    for program in programs.iter() {
        ...
        genesis_config
            .accounts
            .remove(&sonoma_sdk::feature_set::disable_deprecated_loader::id())
            .unwrap();
        genesis_config
            .accounts
            .remove(&sonoma_sdk::feature_set::disable_deploy_of_alloc_free_syscall::id())
            .unwrap();
        let mut bank = Bank::new_for_tests(&genesis_config);
...
fn test_sol_alloc_free_no_longer_deployable() {
    ...
    bank.deactivate_feature(&sonoma_sdk::feature_set::disable_deprecated_loader::id());
    let (name, id, entrypoint) = solana_bpf_loader_deprecated_program!();
    bank.add_builtin(&name, &id, entrypoint);
    ...
    bank.deactivate_feature(&sonoma_sdk::feature_set::disable_deploy_of_alloc_free_syscall::id());
    ...
    bank.activate_feature(&sonoma_sdk::feature_set::disable_deploy_of_alloc_free_syscall::id());
    bank.clear_signatures();
...
fn test_program_bpf_invoke_sanity() {
    ...
    for program in programs.iter() {
        ...
        let account_metas = vec![
            ...
            AccountMeta::new_readonly(sonoma_sdk::ed25519_program::id(), false),
...
fn test_program_bpf_realloc() {
    ...
    assert_eq!(&sonoma_sdk::system_program::id(), account.owner());
    let data = bank_client.get_account_data(&pubkey).unwrap().unwrap();
                ...
                Message::new(
                    &[Instruction::new_with_bytes(
                        ...
                        vec![
                            ...
                            AccountMeta::new(sonoma_sdk::system_program::id(), false),
                        ],
                    )],
                    Some(&mint_pubkey),
                )
                ...
                Message::new(
                    &[Instruction::new_with_bytes(
                        ...
                        vec![
                            AccountMeta::new(pubkey, true),
                            AccountMeta::new_readonly(realloc_program_id, false),
                            AccountMeta::new_readonly(sonoma_sdk::system_program::id(), false),
                        ],
                    )],
                    Some(&mint_pubkey),
                )

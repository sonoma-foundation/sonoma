...
mod tests {
    use {
        ...
        bincode::serialize,
        sonoma_program_runtime::invoke_context::mock_process_instruction,
        sonoma_sdk::{
    ...
    fn create_test_account() -> (Pubkey, AccountSharedData) {
        ...
        (
            vote_pubkey,
            vote_state::create_account(&vote_pubkey, &sonoma_sdk::pubkey::new_rand(), 0, balance),
        )
    }

    fn create_test_account_with_authorized() -> (Pubkey, Pubkey, Pubkey, AccountSharedData) {
        let vote_pubkey = sonoma_sdk::pubkey::new_rand();
        let authorized_voter = sonoma_sdk::pubkey::new_rand();
        let authorized_withdrawer = sonoma_sdk::pubkey::new_rand();
    ...
    fn test_initialize_vote_account() {
        ...
        let node_pubkey = sonoma_sdk::pubkey::new_rand();
    ...
    fn test_authorize_voter() {
        let (vote_pubkey, vote_account) = create_test_account();
        let authorized_voter_pubkey = sonoma_sdk::pubkey::new_rand();
        let clock = Clock {
            epoch: 1,
    ...
    fn test_authorize_withdrawer() {
        let (vote_pubkey, vote_account) = create_test_account();
        let authorized_withdrawer_pubkey = sonoma_sdk::pubkey::new_rand();
        let instruction_data = serialize(&VoteInstruction::Authorize(
